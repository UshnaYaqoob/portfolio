/**
 * ProjectCard — Alternating Left/Right Layout
 *
 * Each project alternates between:
 *  - Left: image, Right: content
 *  - Right: image, Left: content
 *
 * Props:
 *  - layout: "left" or "right" (controls image position)
 */
            ...
            <div className={`flex flex-col gap-4 ${isLeft ? "md:order-2" : "md:order-1"}`}>
                ...
                {Icon && (
                    <div className="inline-flex items-center gap-2 w-fit px-3 py-1 rounded-full bg-blue-600/20 border border-blue-500/30">
                        ...
                        <span className="text-blue-300 text-xs font-semibold">{badge}</span>
                    </div>
                )}

                {/* Title */}
                <h3 className="text-2xl md:text-3xl font-bold text-slate-100">{title}</h3>
